...
/**
 * ThinkEx - DDD Code Generation Tool
 *
 * This class is the core of the DDD code generation system.
 * It reads templates from the demo module and generates
 * code files for new modules.
 *
 * @author blankkids
 * @version 1.0.0
 */
class ThinkEx
{
    /** @var string Line break marker */
    protected $r = "~r";
    /** @var string Newline marker */
    protected $n = "~n";
    /** @var string Tab marker */
    protected $t = "~t";
    /** @var string Space marker */
    protected $s = "~s";

    /** @var array Configuration options */
    protected $opt;
    /** @var string Module demo name */
    protected $moduleDomeName;
    /** @var string Child demo name */
     ...
     * Constructor
     *
     * @param array $option Configuration options
     */
    public function __construct($option)
    {
        ...
        $this->opt            = $option;
        // 设置模板路径
        $this->templatePath = isset($option['demo_template_path']) && !empty($option['demo_template_path'])
            ? $option['demo_template_path']
            : $option['domain_path'];
    }
...
//#########################################################
    ...
    function setDocFile($moduleName, $childName, $dbOn = null)
    {
        $moduleDomeName = $this->moduleDomeName;
        $childDemoName  = $this->childDemoName;
        $templatePath   = $this->getTemplatePath(); // 模板目录
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\" . "doc.md",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\" . "doc.md",
        ];
    ...
    function setControllerFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\port\\controller\\" . ucwords($childDemoName) . "Controller.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\port\\controller\\" . ucwords($childName) . "Controller.php",
        ];
    ...
    function setCmdFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\console\\" . ucwords($childDemoName) . "Cmd.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\console\\" . ucwords($childName) . "Cmd.php",
        ];
    ...
    function setSaveJobFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\job\\" . ucwords($childDemoName) . "SaveJob.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\job\\" . ucwords($childName) . "SaveJob.php",
        ];
    ...
    function setRequestFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\port\\request\\" . ucwords($childDemoName) . "Request.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\port\\request\\" . ucwords($childName) . "Request.php"
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setRepositoryFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\repository\\" . ucwords($childDemoName) . "Repo.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\repository\\" . ucwords($childName) . "Repo.php"
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setModelFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\model\\" . ucwords($childDemoName) . "Model.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\model\\" . ucwords($childName) . "Model.php"
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setTransFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\port\\trans\\" . ucwords($childDemoName) . "Trans.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\port\\trans\\" . ucwords($childName) . "Trans.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setSrvFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\srv\\" . ucwords($childDemoName) . "Srv.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\srv\\" . ucwords($childName) . "Srv.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            $this->fileEditChange($option, function ($option, $demoContent, $changeContent) use ($opt, $dbOn, $dbOptStr) {
    ...
    function setLogicFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\port\\logic\\" . ucwords($childDemoName) . "Logic.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\port\\logic\\" . ucwords($childName) . "Logic.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            $this->fileEditChange($option, function ($option, $demoContent, $changeContent) use ($opt, $dbOn, $dbOptStr) {
    ...
    function setEnumFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\enum\\" . ucwords($childDemoName) . "Enum.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\enum\\" . ucwords($childName) . "Enum.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setErrorFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\error\\" . ucwords($moduleDomeName) . "RootError.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\error\\" . ucwords($moduleName) . "RootError.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setAttributeFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\Resources\\lang\\zh-CN\\" . strtolower($childDemoName) . "-attribute.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\Resources\\lang\\zh-CN\\" . strtolower($this->childNameFilter($childName)) . "-attribute.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setMessageFile($moduleName, $childName, $dbOn = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $templatePath . "\\" . $moduleDomeName . "\\Resources\\lang\\zh-CN\\" . strtolower($childDemoName) . "-message.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\Resources\\lang\\zh-CN\\" . strtolower($this->childNameFilter($childName)) . "-message.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setRoute($moduleName, $childName, $dbOn = null)
    {
        ...
        //默认-配置路径 - 源模板从本包 stubs/route/ 读取，输出到目标项目route/
        $currGetPath = dirname(__DIR__, 2) . "\\stubs\\route\\" . ($moduleDomeName) . ".php";
        $currPutPath = $root . "\\route\\" . ($moduleName) . ".php";

        //兼容多应用模式-需要在配置里注明
        $auto_multi_app = $this->opt['auto_multi_app'];
        if ($auto_multi_app) {
            //多应用模式-配置路径，源模板从本包 stubs/route/ 读取
            $currGetPath = dirname(__DIR__, 2) . "\\stubs\\route\\" . ($moduleDomeName) . ".php";
            $currPutPath = $outputPath . "\\" . $moduleName . "\\route\\" . ($moduleName) . ".php";
        }
        ...
        if ($isExistFile) {
            ...
            // 确保目标 route 目录存在
            $routeDir = dirname($option['put_path']);
            if (!is_dir($routeDir)) {
    ...
    function changeModelFields($moduleName, $childName, $dbOptStr = null)
    {
        ...
        $outputPath     = $this->getOutputPath();   // 输出目录
        $option         = [
            "get_path"    => $outputPath . "\\" . $moduleName . "\\model\\" . ucwords($childName) . "Model.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $outputPath . "\\" . $moduleName . "\\model\\" . ucwords($childName) . "Model.php",
        ];
