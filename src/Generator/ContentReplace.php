...
class ContentReplace
{
    ...
    {
        // 如果已经包含下划线，直接返回小写
        if (strpos($childName, '_') !== false) {
            return strtolower($childName);
        }
    ...
    /**
     * Get default test value for a field based on its type and name
     *
     * @param array $field Field metadata
     * @return mixed Default value for testing
     */
    protected function getTestFieldDefaultValue($field)
    {
        $nativeType = $field['native_type'];
        $fieldName = $field['name'];

        // Determine default value based on native type
        switch ($nativeType) {
            case "LONGLONG":
            case "LONG":
            case "TINY":
    ...
    /**
     * Get field default value with special handling for common fields
     *
     * @param array $field Field metadata
     * @return string Field value expression
     */
    protected function getTestFieldValue($field)
    {
        $defaultValue = $this->getTestFieldDefaultValue($field);
        $fieldName = $field['name'];
